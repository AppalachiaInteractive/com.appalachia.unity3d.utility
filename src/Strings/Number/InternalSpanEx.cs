    ...
    internal static class InternalSpanEx
    {
        ...
        private static bool EqualsOrdinalIgnoreCase(ref char charA, ref char charB, int length)
        {
            ...
            if (IntPtr.Size == 8)
            {
                ...
                while ((uint)length >= 4)
                {
                    var valueA = Unsafe.ReadUnaligned<ulong>(
                        ref Unsafe.As<char, byte>(ref Unsafe.AddByteOffset(ref charA, byteOffset))
                    );
                    var valueB = Unsafe.ReadUnaligned<ulong>(
            ...
            while ((uint)length >= 2)
            {
                var valueA = Unsafe.ReadUnaligned<uint>(
                    ref Unsafe.As<char, byte>(ref Unsafe.AddByteOffset(ref charA, byteOffset))
                );
                var valueB = Unsafe.ReadUnaligned<uint>(
                    ref Unsafe.As<char, byte>(ref Unsafe.AddByteOffset(ref charB, byteOffset))
                );
            ...
            return EqualsOrdinalIgnoreCaseNonAscii(
        ...
        internal static bool UInt64OrdinalIgnoreCaseAscii(ulong valueA, ulong valueB)
        {
            ...
            // the 0x80 bit of each word of 'lowerIndicator' will be set iff the word has value >= 'A'
            var lowerIndicator = (valueA + 0x0080_0080_0080_0080ul) - 0x0041_0041_0041_0041ul;

            // the 0x80 bit of each word of 'upperIndicator' will be set iff (word | 0x20) has value <= 'z'
            var upperIndicator = ((valueA | 0x0020_0020_0020_0020ul) + 0x0100_0100_0100_0100ul) -
                                 0x007B_007B_007B_007Bul;

            // the 0x20 bit of each word of 'combinedIndicator' will be set iff the word is [A-Za-z]
            var combinedIndicator = (0x0080_0080_0080_0080ul & lowerIndicator & upperIndicator) >> 2;
        ...
        private static bool EqualsOrdinalIgnoreCaseNonAscii(ref char charA, ref char charB, int length)
        {
            ...
            //else
            {
                ...
                while (length != 0)
                {
                    ...
                    if ((valueA == valueB) ||
                        (((valueA | 0x20) == (valueB | 0x20)) && (((valueA | 0x20) - 'a') <= 'z' - 'a')))
                    {
                        byteOffset += 2;
                        length--;
                    }
