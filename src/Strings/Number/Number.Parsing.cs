...
namespace Appalachia.Utility.Strings.Number
{
    ...
    internal static partial class Number
    {
        ...
        {
            var status = TryParseInt32(value, styles, info, out var result);
        ...
        {
            var status = TryParseInt64(value, styles, info, out var result);
            if (status != ParsingStatus.OK)
            {
        ...
        {
            var status = TryParseUInt32(value, styles, info, out var result);
            if (status != ParsingStatus.OK)
            {
        ...
        {
            var status = TryParseUInt64(value, styles, info, out var result);
            if (status != ParsingStatus.OK)
            {
                     ...
                     (info.NumberNegativePattern != 2)))
                {
                    if (((styles & NumberStyles.AllowLeadingSign) != 0) &&
                        ...
                        (((next = MatchChars(p, strEnd, info.PositiveSign)) != null) ||
                         (((next = MatchChars(p, strEnd, info.NegativeSign)) != null) &&
            ...
            out int result)
        {
            ...
            HasTrailingChars
                : // we've successfully parsed, but there are still remaining characters in the span
            ...
            out uint result)
        {
            ...
            if (IsDigit(num))
            {
                ...
                // Potential overflow now processing the 10th digit.
                overflow |= ((uint)answer > (uint.MaxValue / 10)) ||
            ...
            out uint result)
        {
            ...
            var overflow = false;
            uint answer = 0;
            var charToHexLookup = CharToHexLookup;
            ...
            if (((uint)num < (uint)charToHexLookup.Length) && (charToHexLookup[num] != 0xFF))
            {
                ...
                for (var i = 0; i < 7; i++) // next 7 digits can't overflow
                {
                    ...
                    if (((uint)num >= (uint)charToHexLookup.Length) ||
                        ((numValue = charToHexLookup[num]) == 0xFF))
                    {
                        goto HasTrailingChars;
                    }
            ...
            out ulong result)
        {
            ...
            if (IsDigit(num))
            {
                ...
                overflow |= ((ulong)answer > (ulong.MaxValue / 10)) ||
        ...
        internal static decimal ParseDecimal(
            ...
            NumberFormatInfo info)
        {
            var status = TryParseDecimal(value, styles, info, out var result);
            if (status != ParsingStatus.OK)
            {
            ...
            out double result)
        {
            ...
            if (!TryStringToNumber(value, styles, ref number, info))
            {
                ...
                else if (valueTrim.StartsWith(
                             info.NegativeSign.AsSpan(),
                             StringComparison.OrdinalIgnoreCase
                         ) &&
                         valueTrim.Slice(info.NegativeSign.Length)
                                  .EqualsOrdinalIgnoreCase(info.NaNSymbol.AsSpan()))
                {
                    result = double.NaN;
                }
                else
            ...
            out float result)
        {
            ...
            if (!TryStringToNumber(value, styles, ref number, info))
            {
                ...
                else if (valueTrim.StartsWith(info.PositiveSign.AsSpan(), StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (!info.PositiveInfinitySymbol.StartsWith(
                            info.PositiveSign,
                            StringComparison.OrdinalIgnoreCase
                        ) &&
                        valueTrim.EqualsOrdinalIgnoreCase(info.PositiveInfinitySymbol.AsSpan()))
                    {
                        result = float.PositiveInfinity;
                    }
                    else if (!info.NaNSymbol.StartsWith(
                                 info.PositiveSign,
                                 StringComparison.OrdinalIgnoreCase
                             ) &&
                             valueTrim.EqualsOrdinalIgnoreCase(info.NaNSymbol.AsSpan()))
                    {
                        result = float.NaN;
                    }
                    else
                    ...
                    (valueTrim.StartsWith(info.NegativeSign.AsSpan(), StringComparison.OrdinalIgnoreCase) &&
        ...
        {
            return (ch == 0x20) || ((uint)(ch - 0x09) <= (0x0D - 0x09)) ? true : false;
        }
        ...
        internal static void ThrowOverflowOrFormatException(ParsingStatus status, TypeCode type = 0)
        {
        ...
        internal static double NumberToDouble(ref NumberBuffer number)
        {
            ...
            {
                var bits = NumberToFloatingPointBits(ref number, in FloatingPointInfo.Double);
        ...
        internal static float NumberToSingle(ref NumberBuffer number)
        {
            ...
            {
                var bits = (uint)NumberToFloatingPointBits(ref number, in FloatingPointInfo.Single);
