...
namespace Appalachia.Utility.DataStructures.Hashing
{
    /// <summary>
    ...
    public class UniversalHashingFamily
    {
        ...
        public UniversalHashingFamily(int numberOfHashFunctions)
        {
            if (numberOfHashFunctions <= 0)
            {
                throw new ArgumentOutOfRangeException(
                    "Number of hash functions should be greater than zero."
                );
            }
        ...
        public void GenerateNewFunctions()
        {
            ...
            var randomMin = 0;
            var randomMax = _primes.Count - 1;

            for (var i = 0; i < _numberOfHashFunctions; i++)
            {
                // Get only the primes that are smaller than the biggest-chosen prime.
                var randomIndex = _randomizer.Next(randomMin, randomMax);
                ...
                while ((_primes[randomIndex] >= BIG_PRIME) ||
                       (_primes[randomIndex] == _firstMultipliersVector[i]))
                {
                    randomIndex = _randomizer.Next(randomMin, randomMax);
                }
        ...
        public int UniversalHash(int preHashedKey, int whichHashFunction)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(
                    "WhichHashFunction parameter should be greater than zero or equal to the number of Hash Functions."
                );
            }

            var a = _firstMultipliersVector[whichHashFunction - 1];
            var b = _secondMultipliersVector[whichHashFunction - 1];
