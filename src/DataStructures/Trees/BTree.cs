...
namespace Appalachia.Utility.DataStructures.Trees
{
    ...
    public class BTreeNode<T> : IComparable<BTreeNode<T>>
        where T : IComparable<T>
    {
        ...
        public void InsertNonFull(T value)
        {
            if (IsLeaf)
            {
                ...
                var i = Keys.FindLastIndex(delegate(T compare) { return value.CompareTo(compare) > 0; }) + 1;
                Keys.Insert(i, value);
            }
            else
            {
        ...
        #region IComparable<BTreeNode<T>> Members
