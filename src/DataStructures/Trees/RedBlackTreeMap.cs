...
namespace Appalachia.Utility.DataStructures.Trees
{
        ...
        where TKey : IComparable<TKey>
    {
        ...
        public override void Insert(TKey key, TValue value)
        {
            ...
            if (!newNode.IsEqualTo(Root))
            {
                if (newNode.Parent.Color !=
                    RedBlackTreeColors.Black) // Case 0: Parent is not black and we have to restructure tree
                {
        ...
        protected virtual void _adjustTreeAfterInsertion(RedBlackTreeMapNode<TKey, TValue> currentNode)
        {
            ...
            if ((currentNode != null) &&
                (currentNode != Root) &&
                _safeCheckIsRed(_safeGetParent(currentNode)))
            {
                //
                // STEP 2.A:
            ...
            while ((currentNode != null) && (currentNode != Root) && currentNode.IsBlack)
            {
                if (currentNode.IsLeftChild)
                {
                    ...
                    if (_safeCheckIsBlack(_safeGetLeftChild(sibling)) &&
                        _safeCheckIsBlack(_safeGetRightChild(sibling)))
                    {
        ...
        * The following are methods for rotating the tree (left/right) and for adjusting the 
        ...
        protected virtual void _rotateLeftAt(RedBlackTreeMapNode<TKey, TValue> currentNode)
        {
            ...
            var isRootNode = currentNode == Root;
        ...
        protected virtual void _safeUpdateColor(
            RedBlackTreeMapNode<TKey, TValue> node,
            RedBlackTreeColors color)
        {
            if (node == null)
            {
                return;
        ...
        * The following are helper methods for safely checking, getting and updating possibly-null objects.
        ...
        protected RedBlackTreeMapNode<TKey, TValue> _safeGetGrandParent(
            RedBlackTreeMapNode<TKey, TValue> node)
        {
