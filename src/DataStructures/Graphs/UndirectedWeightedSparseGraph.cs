...
{
    public class UndirectedWeightedSparseGraph<T> : IGraph<T>, IWeightedGraph<T>
        where T : IComparable<T>
    {
        ...
        protected virtual WeightedEdge<T> _tryGetEdge(T source, T destination)
        {
            ...
            var sourceToDestinationPredicate = new Predicate<WeightedEdge<T>>(
                item => item.Source.IsEqualTo(source) && item.Destination.IsEqualTo(destination)
            );
            var destinationToSourcePredicate = new Predicate<WeightedEdge<T>>(
                item => item.Source.IsEqualTo(destination) && item.Destination.IsEqualTo(source)
            );
