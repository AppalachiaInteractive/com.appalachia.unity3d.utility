...
namespace Appalachia.Utility.DataStructures.Graphs
{
        ...
        where T : IComparable<T>, IEquatable<T>
    {
        ...
        private static V Pick<V>(IEnumerable<V> Set)
        {
        ...
        public class Clique : HashSet<T>, IComparable<Clique>
        {
            ...
            #region IComparable<CliqueGraph<T>.Clique> Members
        ...
        /// </summary>
        private ICollection<T> _vertices = new HashSet<T>();

        /// <summary>
        ///     A set of cliques minimal with the hability of charaterize the graph.
        /// </summary>
        private ISet<Clique> _cliques = new HashSet<Clique>();
        ...
        public CliqueGraph(IEnumerable<T> vertices) : this()
        {
            if (vertices == null)
            {
                System.Diagnostics.Debug.WriteLine(
                    "Cannot initialize an instance of a CliqueGraph with NULL vertices;\ninvoking default constructor."
                );
            }
            else
            {
	...
	*/
            ...
            var H = getPairs(vertices);

            foreach (var clan in _cliques)
            {
                var exc = getPairs(clan);
                H.ExceptWith(exc);
            }
        ...
        private static bool IsComplete(IGraph<T> graph, ISet<T> vertices)
        {
        ...
        private void ExpandToMaximal(Clique clan)
        {
            ...
            var clone = new HashSet<Clique>(_cliques);
            clone.Remove(clan);
        ...
        /// <param name="clan">Clique to expand.</param>
        private static void ExpandToMaximal(IGraph<T> graph, Clique clan)
        {
        ...
        /// </summary>
        private class PairComparer : IEqualityComparer<UnordererPair<T>>
        {
        ...
        private ISet<UnordererPair<T>> getPairs(ICollection<T> vertices)
        {
            var arr = new T[vertices.Count];
        ...
        public IEnumerable<IEdge<T>> Edges
        {
            get
            {
                var returnEdges = new List<UnweightedEdge<T>>();
        ...
        {
            var incomingEdges = new List<UnweightedEdge<T>>();
        ...
        {
            var outgoingEdges = new List<UnweightedEdge<T>>();
        ...
        public bool AddEdge(T firstVertex, T secondVertex)
        {
            ...
            var NewClique = new Clique(); // The new clique that contains the edge (firstVertex, secondVertex)
        ...
        {
            var ret = !_vertices.Contains(vertex);
        ...
        /// </summary>
        /// <returns></returns>
        private ICollection<UnordererPair<T>> getEdges()
        {
            ISet<UnordererPair<T>> H = new HashSet<UnordererPair<T>>();

            foreach (var clan in _cliques)
            {
                var union = getPairs(clan);
                H.UnionWith(union);
            }
        ...
        public int cliqueNumber => Pick(getMaximumCliques).Count;
        ...
        public IGraph<Clique> buildDualGraph()
        {
            ...
            foreach (var clan0 in _cliques)
            {
                ...
                {
                    if (!clan0.Equals(clan1) &&
                        clan0.Overlaps(clan1)) // Equals = SetEquals here since cliques are maximal.
                    {
        ...
        where T : IEquatable<T>
    {
        ...
        #region IEquatable<UnordererPair<T>> Members
