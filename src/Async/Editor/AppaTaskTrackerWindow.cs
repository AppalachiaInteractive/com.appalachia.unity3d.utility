...
{
    public class AppaTaskTrackerWindow : EditorWindow
    {
        private static int interval;

        private static AppaTaskTrackerWindow window;

        [MenuItem("Window/AppaTask Tracker")]
        public static void OpenWindow()
        {
            ...
            GetWindow<AppaTaskTrackerWindow>("AppaTask Tracker").Show();
        }

        private static readonly GUILayoutOption[] EmptyLayoutOption = new GUILayoutOption[0];

        private AppaTaskTrackerTreeView treeView;
        ...
        private void OnEnable()
        {
            ...
            treeView = new AppaTaskTrackerTreeView();
            TaskTracker.EditorEnableState.EnableAutoReload =
                EditorPrefs.GetBool(TaskTracker.EnableAutoReloadKey, false);
            TaskTracker.EditorEnableState.EnableTracking =
                EditorPrefs.GetBool(TaskTracker.EnableTrackingKey, false);
        ...
        private static readonly GUIContent ReloadHeadContent =
            EditorGUIUtility.TrTextContent("Reload", "Reload View.");

        private static readonly GUIContent GCHeadContent =
            EditorGUIUtility.TrTextContent("GC.Collect", "Invoke GC.Collect.");

        private static readonly GUIContent EnableTrackingHeadContent = EditorGUIUtility.TrTextContent(
            "Enable Tracking",
            "Start to track async/await AppaTask. Performance impact: low"
        );

        private static readonly GUIContent EnableStackTraceHeadContent = EditorGUIUtility.TrTextContent(
            "Enable StackTrace",
        ...
        private void RenderDetailsPanel()
        {
            ...
            if (selected.Count > 0)
            {
                ...
                var item =
                    treeView.CurrentBindingItems.FirstOrDefault(
                        x => x.id == first
                    ) as AppaTaskTrackerViewItem;
                if (item != null)
                {
