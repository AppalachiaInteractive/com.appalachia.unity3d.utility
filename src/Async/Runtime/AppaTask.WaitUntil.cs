...
namespace Appalachia.Utility.Async
{
    ...
    {
        public static AppaTask WaitUntil(
            Func<bool> predicate,
            PlayerLoopTiming timing = PlayerLoopTiming.Update,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            return new AppaTask(
                WaitUntilPromise.Create(predicate, timing, cancellationToken, out var token),
                token
            );
        }

        public static AppaTask WaitWhile(
            Func<bool> predicate,
            PlayerLoopTiming timing = PlayerLoopTiming.Update,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            return new AppaTask(
                WaitWhilePromise.Create(predicate, timing, cancellationToken, out var token),
                token
            );
        }

        public static AppaTask WaitUntilCanceled(
            CancellationToken cancellationToken,
            PlayerLoopTiming timing = PlayerLoopTiming.Update)
        {
            return new AppaTask(
                WaitUntilCanceledPromise.Create(cancellationToken, timing, out var token),
                token
            );
        }

        public static AppaTask<U> WaitUntilValueChanged<T, U>(
            T target,
        ...
        private sealed class WaitUntilPromise : IAppaTaskSource,
                                                IPlayerLoopItem,
                                                ITaskPoolNode<WaitUntilPromise>
        {
        ...
        private sealed class WaitWhilePromise : IAppaTaskSource,
                                                IPlayerLoopItem,
                                                ITaskPoolNode<WaitWhilePromise>
        {
        ...
        private sealed class WaitUntilCanceledPromise : IAppaTaskSource,
                                                        IPlayerLoopItem,
        ...
        private sealed class WaitUntilValueChangedUnityObjectPromise<T, U> : IAppaTaskSource<U>,
            IPlayerLoopItem,
            ITaskPoolNode<WaitUntilValueChangedUnityObjectPromise<T, U>>
        {
            ...
            static WaitUntilValueChangedUnityObjectPromise()
            {
                TaskPool.RegisterSizeGetter(
                    typeof(WaitUntilValueChangedUnityObjectPromise<T, U>),
                    () => pool.Size
                );
            }
            ...
            private AppaTaskCompletionSourceCore<U> core;

            private WaitUntilValueChangedUnityObjectPromise()
            {
            }

            public static IAppaTaskSource<U> Create(
                T target,
                Func<T, U> monitorFunction,
                ...
                out short token)
            {
                if (cancellationToken.IsCancellationRequested)
                {
                    return AutoResetAppaTaskCompletionSource<U>.CreateFromCanceled(
                        cancellationToken,
                        out token
                    );
                }
        ...
        private sealed class WaitUntilValueChangedStandardObjectPromise<T, U> : IAppaTaskSource<U>,
            IPlayerLoopItem,
            ...
            where T : class
        {
            ...
            {
                TaskPool.RegisterSizeGetter(
                    typeof(WaitUntilValueChangedStandardObjectPromise<T, U>),
                    () => pool.Size
                );
            }
