...
namespace Appalachia.Utility.Async.Internal
{
    internal static class DiagnosticsExtensions
    {
        ...
        private static readonly Dictionary<Type, string> builtInTypeNames = new Dictionary<Type, string>
        {
            ...
            { typeof(Task), "Task" },
            { typeof(AppaTask), "AppaTask" },
            { typeof(AppaTaskVoid), "AppaTaskVoid" }
        };
        ...
        private static string BeautifyType(Type t, bool shortName)
        {
            ...
            if (!t.IsGenericType)
            {
                return shortName
                    ? t.Name
                    : t.FullName.Replace("Appalachia.Utility.Async.Triggers.", "")
                       .Replace("Appalachia.Utility.Async.Internal.", "")
            ...
            return typeBeautifyRegex.Replace(genericType, "")
                                    .Replace("Appalachia.Utility.Async.Triggers.", "")
                                    .Replace("Appalachia.Utility.Async.Internal.", "")
                                    .Replace("Appalachia.Utility.Async.",          "") +
                   "<" +
                   innerFormat +
        ...
        private static bool IgnoreLine(MethodBase methodInfo)
        {
            ...
            if (declareType.StartsWith("Appalachia.Utility.Async.CompilerServices"))
            {
                return true;
            }
            ...
            if (declareType.StartsWith("Appalachia.Utility.Async.AppaTaskCompletionSourceCore"))
            {
                return true;
            }

            if (declareType.StartsWith("Appalachia.Utility.Async.AwaiterActions"))
            {
                return true;
            }
