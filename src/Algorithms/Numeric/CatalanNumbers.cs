...
namespace Appalachia.Utility.Algorithms.Numeric
{
    public static class CatalanNumbers
    {
        ...
        /// </summary>
        private static readonly Dictionary<uint, BigInteger> CachedCatalanNumbers =
            new Dictionary<uint, BigInteger> { { 0, 1 }, { 1, 1 } };
        ...
        private static BigInteger _recursiveHelper(uint rank)
        {
            ...
            for (uint i = 0; i <= lastRank; ++i)
            {
                ...
                if (!CachedCatalanNumbers.ContainsKey(i))
                {
                    CachedCatalanNumbers.Add(i, firstPart);
                }

                if (!CachedCatalanNumbers.ContainsKey(lastRank - i))
                {
                    CachedCatalanNumbers.Add(lastRank - i, secondPart);
                }
