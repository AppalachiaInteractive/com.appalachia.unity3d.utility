...
namespace Appalachia.Utility.Algorithms.Graphs
{
    public static class DepthFirstSearcher
    {
        ...
        /// </summary>
        public static T FindFirstMatch<T>(IGraph<T> Graph, T StartVertex, Predicate<T> Match)
            where T : IComparable<T>
        {
            ...
            var parents =
                new Dictionary<T, object>(Graph.VerticesCount); // keeps track of visited nodes and tree-edges
        ...
        /// </summary>
        public static void PrintAll<T>(IGraph<T> Graph, T StartVertex)
            where T : IComparable<T>
        {
            // Check if graph is empty
            if (Graph.VerticesCount == 0)
        ...
        /// </summary>
        public static void VisitAll<T>(ref IGraph<T> Graph, T StartVertex, Action<T> Action)
            where T : IComparable<T>
        {
            // Check if graph is empty
            if (Graph.VerticesCount == 0)
        ...
        /// </summary>
        private static void _visitNeighbors<T>(
            T Vertex,
            ref IGraph<T> Graph,
            ref Dictionary<T, object> Parents,
            Action<T> Action)
            where T : IComparable<T>
        {
